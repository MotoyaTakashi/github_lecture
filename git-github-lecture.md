...
2. 「Sign up」をクリック
3. メールアドレス・パスワード・ユーザ名・あなたの国/地域を登録
4. メール認証を実施
...
SSHキーの作成は **Ubuntu（WSL含む）** のターミナルで行います。
...
実行すると、ホームディレクトリ以下に以下の2つが作成されます：
- **公開鍵**（GitHubに登録してよい鍵）
- **秘密鍵**（あなたのPCだけが持つ鍵・絶対に他人に見せない）
...
### ④ GitHubにSSHキーを登録（Ubuntuで作成した公開鍵を使用）
